...
public class caregiverReg extends AppCompatActivity {
                ...
                if(pw_s.length() >= 6){
                    ...
                    if(!name_s.isEmpty() && !address_s.isEmpty() && !phoneNumber_s.isEmpty() && !NIC_s.isEmpty() && !birthDate_s.isEmpty() && !adminIdk_s.isEmpty() && !email_s.isEmpty() && !pw_s.isEmpty() && !patientId_s.isEmpty()){
                        if(checkUserType(adminIdk_s)){
                            ...
                            Toast.makeText(caregiverReg.this, "Please enter username starting with 'c_'",
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            Toast.makeText(caregiverReg.this, "Registration successful.",
                                    Toast.LENGTH_SHORT).show();
                            clear(name, address, phoneNumber, NIC, birthDate, email, adminID, password, patientID);
    ...
    private boolean checkUserType(String userid){
        boolean dtype;
        if(Character.toLowerCase(userid.charAt(0)) == 'c' && userid.charAt(1) == '_'){
            dtype = true;
        }else{
            dtype = false;
        }
    ...
    private void break_(String adminId){
        String adminId_ = adminId.replace("c_", "");
    }

    private void clear(EditText name, EditText address, EditText phoneNumber, EditText NIC, EditText birthDate, EditText email, EditText adminID, EditText password, EditText patientID){
        name.setText("");
        address.setText("");
        phoneNumber.setText("");
        NIC.setText("");
